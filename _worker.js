...
// ── Site Filtering (blocked domains) & Visit Log ─────────────────────────────
const BLOCK_TTL = 30000; // re-read the blocklist at most every 30s
let _blockCache = { at: 0, list: [] };
async function getBlocklist(env) {
  const now = Date.now();
  if (now - _blockCache.at < BLOCK_TTL && _blockCache.list) return _blockCache.list;
  let list = [];
  try {
    const raw = await env.SPIDER_KV.get('settings');
    if (raw) {
      const s = JSON.parse(raw);
      if (Array.isArray(s.blocked_domains)) list = s.blocked_domains.map(String).filter(Boolean);
    }
  } catch (e) {}
  _blockCache = { at: now, list };
  return list;
}
function domainBlocked(host, list) {
  if (!host || !list || !list.length) return false;
  host = String(host).toLowerCase().replace(/\.$/, '');
  for (let d of list) {
    d = String(d || '').trim().toLowerCase().replace(/^\.+/, '').replace(/\.$/, '');
    if (!d) continue;
    if (d.startsWith('*.')) {
...
const VISIT_MAX = 100;
const VISIT_DEBOUNCE_MS = 30000;
const _visitDebounce = new Map(); // `${uuid}|${host}` -> ts
async function logVisit(env, uuid, host) {
  if (!uuid || !host) return;
  const key = uuid + '|' + host;
  const now = Date.now();
  if (now - (_visitDebounce.get(key) || 0) < VISIT_DEBOUNCE_MS) return;
  _visitDebounce.set(key, now);
  try {
    const k = 'visits:' + uuid;
    const raw = await env.SPIDER_KV.get(k);
    let list = raw ? JSON.parse(raw) : [];
    list.push({ host: String(host).toLowerCase(), at: now });
    if (list.length > VISIT_MAX) list = list.slice(-VISIT_MAX);
    await env.SPIDER_KV.put(k, JSON.stringify(list));
  } catch (e) {}
}
...
async function handleVlessWs(request, env, country, preUser) {
  ...
  server.addEventListener('message', async (ev) => {
    ...
    if (!server.__h) {
      ...
      // Site filtering: reject connections to blocked domains
      if (await domainBlocked(h.address, await getBlocklist(env))) {
        try { server.close(4033, 'blocked domain'); } catch(e){}
        return;
      }
      // Visit log: record which sites this user connects to
      await logVisit(env, user.uuid, h.address);
...
async function handleTunnelWs(request, env, user, reverse) {
  ...
  server.addEventListener('message', async (ev) => {
    ...
    if (!server.__h) {
      ...
      server.__h = h;
      server.__user = user;
      // Site filtering + visit log (tunnel path)
      if (await domainBlocked(h.address, await getBlocklist(env))) {
        try { server.close(4033, 'blocked domain'); } catch(e){}
        return;
      }
...
// ── Admin API (Bearer PANEL_TOKEN) ──
    ...
    if (path.startsWith('/api/')) {
      ...
      // GET /api/visits/{uuid} — sites a user connected to (site filter log)
      if (path.startsWith('/api/visits/') && request.method === 'GET') {
        const vuid = decodeURIComponent(path.slice('/api/visits/'.length)).toLowerCase();
        const raw = await env.SPIDER_KV.get('visits:' + vuid);
        return json({ ok: true, visits: raw ? JSON.parse(raw) : [] });
      }
